...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        for seq in self.seq_list:
            if not self.data_is_zipped:
                path_to_folder = os.path.join(self.gt_fol, self.gt_set)
                curr_file = self.config["GT_LOC_FORMAT"].format(gt_folder=path_to_folder, seq=seq)
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    # print("tracker fol", self.tracker_fol)
                    # print("tracker", tracker)
                    # print("sub fol ", self.tracker_sub_fol)
                    curr_file = os.path.join(self.tracker_fol, tracker, seq + '_results.txt')
        ...
        if self.config["SEQ_INFO"]:
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    ...
                    seq = row[0]
                    seq_list.append(seq)
                    ini_file = os.path.join(self.gt_fol, self.gt_set, seq, 'seqinfo.ini')
                    if not os.path.isfile(ini_file):
                        raise TrackEvalException('ini file does not exist: ' + seq + '/' + os.path.basename(ini_file))
                    ini_data = configparser.ConfigParser()
                    ini_data.read(ini_file)
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if self.data_is_zipped:
            ...
            if is_gt:
                path_to_folder = os.path.join(self.gt_fol, self.gt_set)
                file = self.config["GT_LOC_FORMAT"].format(gt_folder=path_to_folder, seq=seq)
            else:
                file = os.path.join(self.tracker_fol, tracker, seq + '_results.txt')
