...
def main(config):
    ...
    writer = SummaryWriter(f"runs/{config.num_classes}_{config.num_shot}_{config.random_seed}_{config.hidden_dim}_{config.learning_rate}_shuffle")
    ...
    for step in tqdm(range(config.meta_train_steps)):
        ...
        if (step + 1) % config.eval_freq == 0:
            ...
            with open(f'submission/mann_results_{config.num_shot}_{config.num_classes}_{config.hidden_dim}_{config.learning_rate}.npy', 'wb') as f:
